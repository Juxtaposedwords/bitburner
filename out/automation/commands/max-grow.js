/**  max-grow.js will launch grow.js on the server with as many
...
export async function main(ns) {
	...
	const usage = "WARN: Usage: max-grow.js --server <server> --target <target>"
	...
	if (growThreads < 1) {		
		const nf = ns.nFormat
		ns.tprint(`ERROR: not enough RAM: ${nf(growRam, '0.0a')} needed, ${nf(availRam, '0.0a')} available.`)
		return;
	}
